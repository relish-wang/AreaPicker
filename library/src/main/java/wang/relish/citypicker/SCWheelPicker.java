...
public class SCWheelPicker extends View implements IDebug, IWheelPicker, Runnable {
     ...
     * 数据源
     */
    private List<? extends IPickerModel> mData = new ArrayList<>();
...
//        computeTextSize();
        ...
        for (int drawnDataPos = drawnDataStartPos + mSelectedItemPosition,
            ...
            if (isCyclic) {
                ...
                if (isPosInRang(drawnDataPos)) {
                    data = mData.get(drawnDataPos).getName();
                }
            }
    ...
    @Override
    public List<? extends IPickerModel> getData() {
        return mData;
    }

    @Override
    public void setData(List<? extends IPickerModel> data) {
        if (null == data) {
            data = new ArrayList<>();
        ...
        // 重置位置
        if (mSelectedItemPosition > data.size() - 1 || mCurrentItemPosition > data.size() - 1 || mSelectedItemPosition < 0 || mCurrentItemPosition < 0) {
